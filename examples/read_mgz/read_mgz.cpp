...
int main(int argc, char** argv) {
    ...
    {
        std::unique_ptr< std::ostream > ofs_p = std::unique_ptr< std::ostream >(new zstr::ofstream(write_filename));
        std::ostream * os_p = ofs_p.get();
        fs::write_mgh(mgh, *os_p);
    } // Scope ends: zstr::ofstream is destroyed, flushing and closing the gzip stream.
